...
%\usepackage[cm]{fullpage}%Dont use because not found on mac.
\usepackage{verbatim}
\usepackage{hyperref} 
\usepackage{graphicx}
\usepackage{natbib}
...
for(problem.i in seq_along(problem.list)){
  ...
  max.segments <- floor(min(nrow(pro)/2 + 1, 10))
  break.mat <- if(.Platform$OS.type=="unix"){
    fit <- Segmentor3IsBack::Segmentor(
      pro$logratio, model=2, Kmax=max.segments)
    fit@breaks
  }else{
    ## This should only happen on windows CRAN checks, 
    ## where for some reason there is a problem with Segmentor,
    ## so we use the slow quadratic time 
    ## classic dynamic programming algorithm instead.
    fit <- changepoint::cpt.mean(
      pro$logratio, 
      penalty="None", 
      method="SegNeigh", 
      Q=max.segments)
    ends <- rbind(NA, fit@cpts.full)
    diag(ends) <- nrow(pro)
    ends
  }
...
It is clear from the scatterplot that the learned model 
predicts log(penalty) values which are quite different from the BIC. 
