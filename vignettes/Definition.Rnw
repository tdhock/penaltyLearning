...
Then, we use labelError to compute the number of incorrect labels
for each model, for each labeled data sequence.
...
You can also use featureMatrix for computing a larger feature matrix
in real data sets where you aren't sure what features are relevant.
...
Curve (AUC), which in this case is 1, indicating a function which can
perfectly discriminate between positive and negatve labels in the test
set.

<<ROC>>=
test.pred <- feature.dt[chromosome=="11",]
roc <- ROChange(all.errors, test.pred, c("profile.id", "chromosome"))
...
bic.roc <- ROChange(all.errors, bic.test, c("profile.id", "chromosome"))
print(bic.roc$thresholds[threshold=="predicted"])
@ 

It is clear that the BIC suffers from 3 false positives in chromosome
11. We visualize them along with all the other errors on the other
chromosomes in the plot below,
...
It is clear from the plot above that there are many false positives --
normal labels where there should be no changes, but the BIC model
predicts some.
